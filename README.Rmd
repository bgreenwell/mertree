...
partial_1d(fm, x.name = "time", newdata = simd, .progress = "win") %>%
  plot(type = "l")

# Partial dependence on covariates (notice x3 and x6 are flat!)
par(mfrow = c(3, 2))
for (i in 1:6) {
  partial_1d(fm, x.name = paste0("x", i), newdata = simd, .progress = "win") %>%
    plot(type = "l")
}

# Interaction between x1 and x4
pd_14 <- partial_2d(fm, x1.name = "x1", x2.name = "x4", newdata = simd, .progress = "win",
                    n1 = 20, n2 = 20)
lattice::wireframe(y ~ x1 * x4, data = pd_14, drape = TRUE, colorkey = TRUE)
```
